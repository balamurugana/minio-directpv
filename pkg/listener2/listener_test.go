...
func (handler *testEventHandler) Handle(ctx context.Context, args EventArgs) error {
	if err := handler.handleFunc(args); err != nil {
		handler.t.Fatal(err)
	}
	return nil
}

func startTestController(ctx context.Context, t *testing.T, handler *testEventHandler) {
	hostname, err := os.Hostname()
	if err != nil {
		t.Fatal(err)
	}

	listener := NewListener(handler, "test-volume-controller", hostname, 1)
	go func() {
...
func getHandleFunc(event EventType, volumes ...*directcsi.DirectCSIVolume) (<-chan struct{}, func(EventArgs) error) {
	doneCh := make(chan struct{})
	i := 0
	return doneCh, func(args EventArgs) (err error) {
		defer func() {
			i++
			if err != nil || i == len(volumes) {
				close(doneCh)
			}
		}()

		if args.Event != event {
			err = fmt.Errorf("expected: %v, got: %v", event, args.Event)
			return
		}

		volume := args.Object.(*directcsi.DirectCSIVolume)
		if !reflect.DeepEqual(volumes[i], volume) {
			err = fmt.Errorf("received volume is not equal to volumes[%v]", i)
		}
		return err
	}
}
...
func TestListener(t *testing.T) {
	...
	doneCh, handleFunc := getHandleFunc(AddEvent, volumes...)
	testHandler.handleFunc = handleFunc
	startTestController(ctx, t, testHandler)
	<-doneCh
	...
	doneCh, handleFunc = getHandleFunc(UpdateEvent, volumes[1])
	testHandler.handleFunc = handleFunc
	for _, volume := range volumes {
	...
	doneCh, handleFunc = getHandleFunc(DeleteEvent, volumes...)
	testHandler.handleFunc = handleFunc
	for _, volume := range volumes {
		_, err := testHandler.directCSIClient.DirectV1beta2().DirectCSIVolumes().Update(
			ctx,
